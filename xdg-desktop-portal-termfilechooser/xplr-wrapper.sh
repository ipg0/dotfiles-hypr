...
if [ "$save" = "1" ]; then
	/usr/bin/touch "$path"
	$termcmd -- /bin/bash -c "$cmd --on-load \"SwitchMode: save_file\" -- \"$path\" > \"$out\""
elif [ "$directory" = "1" ]; then
	$termcmd -- /bin/bash -c "$cmd --on-load \"SwitchMode: open_folder\" > \"$out\""
elif [ "$multiple" = "1" ]; then
	$termcmd -- /bin/bash -c "$cmd --on-load \"SwitchMode: open_files\" > \"$out\""
else
	$termcmd -- /bin/bash -c "$cmd --on-load \"SwitchMode: open_file\" > \"$out\""
fi
